...
VERSION=$(curl -s https://api.github.com/repos/brave/brave-browser/releases|grep browser_download_url|grep "nightly-.*-linux-$_arch"|head -1|cut -d- -f5)
ARCHITECTURE=$(uname -m|sed s/x86_/amd/\;s/arch/rm/)
...
echo "We are in ARM64 so getting the last nightly image" 
...
PKG=brave
EXEC_FILE=brave
SRC_URL=https://github.com/brave/brave-browser/releases/download/v${VERSION}/brave-browser-nightly-${VERSION}-linux-${ARCHITECTURE}.zip
